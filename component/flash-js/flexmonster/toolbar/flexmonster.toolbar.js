...
var FlexmonsterToolbar = function(pivotContainerId, pivot, isHTML5, width, labels) {
	...
	self.chartsMultipleHandler = function() {
		...
		if (multiple) {
			document.getElementById("fm-tab-charts-multiple").id = "fm-tab-charts-multiple-selected";
		} else {
			document.getElementById("fm-tab-charts-multiple-selected").id = "fm-tab-charts-multiple";
		}
    ...
    self.createConditionalFormattingItem = function (data, allConditions) {
        ...
        rightContainer.className = "fm-right-container";
        ...
        itemRenderer.appendChild(rightContainer);
    ...
    self.enterFullscreen = function (element) {
    	if (element.requestFullscreen || element.webkitRequestFullScreen
			...
			window["fm-toolbar-height"] = self.toolbarWrapper.getBoundingClientRect().height;
			self.containerStyle = {
	...
	self.init = function(dataProvider) {
        ...
        for (var i = 0; i < dataProvider.length; i++) {
        	if (self.isDisabled(dataProvider[i])) continue;
       		if (isMobile && dataProvider[i].menu != null) {
	...
	self.showDropdown = function() {
		...
		if (menu) {
			...
			if (menu.getBoundingClientRect().right > self.toolbarWrapper.getBoundingClientRect().width) {
				menu.style.right = 0;
